...
export default function MenuBurger(){
    ...
    const dropdownMenuSelector = useAppSelector((state:RootState) => state.window.displayDropdownMenu);
    const dispatch = useAppDispatch();
    
    function toggleMenu(){
        dispatch(toggleDropdownMenu(!dropdownMenuSelector));
    }
    ...
    return(
        <div className="min-w-[40px] min-h-[40px] max-w-[40px] max-h-[40px] mr-[30px] ml-[5px] flex flex-col justify-center gap-[10px] cursor-pointer" onClick={toggleMenu}>
            <span className={`w-full h-[3px] bg-hi transition-all duration-300
                ${dropdownMenuSelector && "translate-y-[13px] rotate-[135deg]"}
            `}/>

            <span className={`w-full h-[3px] bg-hi transition-all duration-300
                ${dropdownMenuSelector && "opacity-0"}
            `}/>
