...
async function filterByTags(supabase:SupabaseClient, tags:string[]):Promise<FilteredIDPromise>{
    ...
    let { data, error } = await supabase.rpc('get_games_by_all_tags', { tags });
...
export async function fetchFilteredGames(filters:FilterQueryParams):Promise<{
    ...
    if(!anyError && filters.aspect && filters.aspect.trim() !== ""){
        let convertedAspect = `${filters.aspect.toLowerCase()}_avg`
        console.log(convertedAspect);
    ...
    if(gameIds.length > 0){
        ...
        let {data, error} = await query.select(`
                *,
                ...
                score:AverageReview(*)
        `).in("id", gameIds)
        if(data && data.length > 0 && gameIds.length > 0){
